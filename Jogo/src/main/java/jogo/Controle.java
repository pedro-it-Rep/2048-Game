...
public class Controle extends JPanel {
  ...
  int pontuacao = 0; //Inica a pontuação com 0
        ...
        System.out.println(newX+","+newY);
              ...
              MovDireita();
                 ...
                 if(deltaX < deltaY){
                  ...
                  MovEsquerda();
                  System.out.println("Diagonal Esquerda");
              }
    }
repaint(); //PintaNovamente o background
  ...
  public void MovEsquerda() {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      if (!adicionaNovaPeca && !comparaLinhas(linhaJogo, juntaJogo)) {
  ...
  private List<EspecPeca> EspacoDisponivel() {
    ...
    for (EspecPeca t : pecas) { //Enquato t = pecas
  ...
  //Compara duas linhas para ver se ja estão completas, se estiverem completas não pode adicionar nenhuma peça
  private boolean comparaLinhas(EspecPeca[] linha1, EspecPeca[] linha2) {
    if (linha1 == linha2) {
      return true;
    ...
    for (int i = 0; i < linha1.length; i++) {
      if (linha1[i].value != linha2[i].value) {
        return false;
      }
    }
    return true;
  ...
  private EspecPeca[] rotacaoImagem(int angulo) {
    ...
    if (angulo == 90) { //Lembrando das aulas fisica, quando o seno tem angulação de 90 graus, ele é 0, n precisando de Y
      offsetY = 0;
    } else if (angulo == 270) { //Lembrando das aulas fisica, quando o cos tem angulação de 270 graus, ele tende a 0, n precisando de X
      offsetX = 0;
    }
  ...
  private void desenhaPeca(Graphics g2, EspecPeca tile, int x, int y) {
    ...
    if (condVitoria || condDerrota) {
      ...
      if (condVitoria) {
        g.drawString("You won!", 80, 150); //Printa a mensagem de vitoria
      }
    ...
    g.drawString("Score: " + pontuacao, 130, 370); //Pontuação do jogo
